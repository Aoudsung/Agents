---
name: paper-search
description: 围绕研究问题、研究方向或种子论文建立可持续修订的中文文献地图、证据关系和分层阅读路径。统一完成问题形式化、按需检索、候选召回、引用谱系、版本身份、论文类型与证据模式假设、阅读价值判断、精读选择、定向外部核查和 reading-return 回灌；通过中性且类型感知的 reading-brief 与 paper-reading 协作。适用于相关工作、survey、反证、引用谱系、最近工作核查和跨论文领域认识构建。
---
...
目标不是堆积论文或把候选塞进统一机制分类，而是建立可核验、可修订、能指导阅读的文献地图。最终应让读者知道：

- 领域试图解决哪些问题；
- 形成了哪些研究传统和证据路线；
- 每篇代表论文增加了什么；
- 哪些证据支持、扩展、限制、挑战或无法比较；
- 为什么先读某篇而不是另一篇；
- 哪些判断仍必须由全文回答。

## 2. 与 Analysis Agent 的边界
...
- A/B/C/D 检索意图的按需启用；
- 候选召回、引用谱系、身份和版本归一；
- 论文类型、证据模式、阅读角色和领域关系的检索级假设；
- 同质簇、代表工作、反证和精读优先级；
- Analysis 提出的 Claim、领域关系或研究机会定向核查；
- 吸收 reading-return 后更新阅读价值与领域地图。

Analysis Agent 负责全文中的研究类型确认、作者论证、研究设计、具体 Claim-Evidence、阅读报告和跨论文知识合成。摘要只能支持候选关系和待核问题，不能证明机制、因果效应、研究质量或新颖性。
...
无法说明时停止。停止表示当前新增结果不再增加研究路线、证据角色、反证、评价协议或关键未知量，不代表检索空间绝对完备。
...
- domain_relation_hypothesis；
- relation_kind：direct、extends、conditions、challenges、measurement、governance、analogue、incomparable；
- priority_reason；
- questions_for_full_text。

这些均为检索级假设，Analysis 可以纠正。不要因为主题相同就假定构念、分析单位或效果可比。
...
- reading_role；
- domain_relation_before 与 domain_relation_after；
- relation_kind；
- 新术语、引用和定向查询；
- invalidated_routes_or_tasks；
- 只需局部重算的部分。

Analysis 将预设错误标成 not-applicable 时，修正 brief 生成逻辑和当前地图；不要把它当作“未回答”。
...
2. 研究传统和证据类型；
3. 研究路线与代表论文；
4. 证据关系：支持、扩展、条件化、挑战、不可比较；
5. 已相对建立的认识与仍有争议的认识；
6. 分层阅读路径；
7. 建议精读及逐题交接；
8. 证据边界和下一动作。

报告必须自包含。P 编号只能辅助定位，不能代替论文标题、具体贡献和证据角色。不要在领域认识形成前跳到系统设计 brainstorm。

## 14. INDEX 与阅读导航

更新 papers/INDEX.md 时至少记录：
...
- why_read；
- domain_relation；
- report 和 reading-return 路径。

INDEX 不只是文件导航表，还应支持“先读哪篇、为什么读、各篇在论证链中承担什么作用”。
...
- 精读反馈实际更新 reading_value 和 domain_relation；
- 路线结论列出论文标题、具体发现、证据类型和关系；
- 不把设计变量、构念、proxy 和最终结果放在同一轴比较；
- 不用统一“中间表示/长期部署/组件消融”语言覆盖异质研究；
- 检索不到不写成不存在；
- 领域报告先形成知识模型，再按需提出研究议程。
